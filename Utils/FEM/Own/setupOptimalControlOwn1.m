function [constraints, objective, xbar, ubar, var] = setupOptimalControlOwn1(A, B, P, Q, R, H, Z, U, Xf, N, xss, uss)
...
% objective = objective +(ubar{1})'*R*(ubar{1})*0.1;
...
% for i = 1:N-1
%     constraints = [constraints, ubar{i+1} - ubar{i}<= 50];
%     constraints = [constraints, ubar{i+1} - ubar{i}>= -50];
% end

% constraints = [constraints, ubar{1}-ubar_in <= 50];
% constraints = [constraints, ubar{1}-ubar_in >= -50];
